...
## ✅ BLOKER ARCHITEKTONICZNY — ROZSTRZYGNIĘTY (werdykt architekta 2026-06-30)
**Werdykt: ścieżka A (wire rich model). Rama A/B była fałszywa — B odpada.** Rich model (`StartEating`/`ConsumeBite`/`StopEating`) to zatwierdzony+zshipowany gate `APPETITE_GRUBAS_design.md`, nie otwarta decyzja; brak okablowania = wiring-debt zalogowany w `MILESTONES.md` ("Wiring-debt: BTTask_Eat → Start/Bite/Stop"). Kasacja (B) wyrzucałaby działający kod niezależny od okablowania → off the table.
- **Rozwiązanie sierocej kolizji nazw:** zamiast przepinać BT na BP-asset, kanoniczny `BTTask_Eat` został przepisany jako **JEDNA klasa C++** łącząca rich appetite z claimem afordancji (commit `0c853b9`). BP `BTTask_Eat.uasset` → do deprecjacji (NIE kasować bez potwierdzenia z PIE). Pełny kontrakt + DoD: **`_gates/BTTASK_EAT_WIRING_01.md`** (nadrzędny nad tym gate'em).
- **Historyczny dowód blokera (zachowany):** import-table `BT_Exploration.uasset` referowała C++ taski z `/Script/Stan_Pierwotny`, BRAK referencji do assetu `/Game/DocelowaGra/AI_NPC/BTTask_Eat` — stąd rich-eat z BP-assetu nigdy nie wykonałby się w PIE. PIE 2026-06-30: NPC zemdleli z wyczerpania (sen) zanim głód dostał turę → osobny ticket `SLEEP_FAINT_TRAP_01` (RESOLVED seedem `Maslow.SeedRestedHungry`).
