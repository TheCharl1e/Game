...
> Zsynchronizowano z gitem 2026-07-01 (reconcile ROADMAP↔rzeczywistość). Domknięte od ostatniego przeglądu:
> L0-02/03/04 flee/panic (03/04 częściowo), L1-06/07/08/08b sen+prędkość, L1-09a/b clothing+fire, L3-05 P2P rdzeń, cały track L0-TA-S1 afordancje.
- ✅ **L3-01 NPCRegistry** ⭐ keystone — DONE 2026-06-20. Odblokowane: P2P (L3-05), reputacja (L4-01), ActionLog/detektyw (L5-01).
- ✅ **L3-02 OCEAN slice #1** — DONE 2026-06-20 (Neurotyczność→panika, ŻYWY W GRZE).
- 🔨 **L0-TA-S1 afordancje** — CORE + EQS + claim PIE-verified (2026-06-29). **ZOSTAJE:** pełna pętla autonomiczna eksploracji (L0-TA-S1e, poddrzewo BT) → potem port do Game_58 (L0-TA-S2).
- 🔨 **L3-05 P2P** — rdzeń PIE-verified; **DECYZJA DYREKTORA:** dodać `UContractTraderComponent` na `BP_NPC_Character` (autonomiczny obrót z potrzeby) → L3-06.
- **L2-03 DataTables** — DT_FoodStats, DT_ItemDefinitions, wpięcie DT_ActionCosts. (Blokuje też L1-10 thirst-scaling — ten sam odpięty drut.)
- **L1-10 thirst activity-scaling** — INERT (sterownik martwy), naprawa = GATE ARCHITEKTA (wołać SetActionByRow z BT/metabolizmu).
- **TECH-01/02 cleanup** — redirectory, duplikaty _Experyment. (TECH-03 literówki BB ✅ done.)
...
| TECH-07 | `GetHPPercent()` dzieli przez literał 100, nie przez `CurrentMaxHP` | ✅ | P2 | — | **Znaleziono 2026-06-20 podczas L3-02.** `MaslowBiologicalComponent.cpp:637-640` zwraca `CurrentHP/100.0` (= AbsoluteMaxHP), NIE `CurrentHP/CurrentMaxHP`. Autofagia/odwodnienie OBNIŻA `CurrentMaxHP` (i `CurrentHP` jest do niego klampowane) → wyniszczony NPC przy PEŁNYM aktualnym zdrowiu czyta `GetHPPercent()=0.5`. Paski HUD błędnie czytają po wyniszczeniu. Fix: dzielić przez `CurrentMaxHP` (guard >0). L3-02 panika świadomie NIE używa tego gettera (liczy własne HP%). **✅ DONE + PIE-verified 2026-06-30** (`REPORTS/raport_2026-06-30_tech07_done.md`): getter dzieli przez `CurrentMaxHP` + guard `<=0→0`; CDO 5/5 (50/50→1.0 vs stary 0.5; 0/80→0.0 guard; 50/25→0.5) + żywa instancja PIE w Game_58. C++-only, build editor-closed Result: Succeeded w obu repo (E:\Game 5.7 + Game_58 5.8). |
...
| L0-02 | Panic override: abort BT task queue on threat | ✅ | P1 | L1-05, L0-04 | **DONE + PIE-verified 2026-06-23 (TASK 3, commit 3335fe2).** Damage-hook→threat→BT interrupt→flee. Gałąź panika/Flee wpięta do korzenia (ożywia L3-02 `bIsInPanic`). Raport `task3_flee_done`. |
| L0-03 | Flee behavior (drop loaded container, run to Safe Zone) | 🔨 | P1 | L0-02, L3-01, L2-12 | **Flee-away DONE + PIE-verified 2026-06-23** (`UBTTask_Flee`, ucieczka OD zagrożenia, commit 826b20e/3335fe2). **ZOSTAJE:** drop-container przy ucieczce (`DropContainer()`) — czeka na L2-12 (jeszcze ⬜). |
| L0-04 | Threat detection → panic (predator via senses/EQS) | 🔨 | P1 | L2-07, SUP-03 | **Threat-hook DONE + PIE-verified 2026-06-23** (generyczne zagrożenie + damage-hook, commit 826b20e). **ZOSTAJE:** wykrywanie DRAPIEŻNIKA przez zmysły/EQS (BP_NPC_WOLF, VisionAcuity gate) — czeka na SUP-03 (percepcja jako dane). Dziś próg = otrzymana rana, nie widok predatora. |

---

## 🌍 L0-TA — ŚWIAT / AFORDANCJE (Track A — „lifeless world + exploration spine")

> Osobny track z briefu `E:\Game_git\L0_TrackA_Slice1_BuildBrief.md`. Świat jako REKORDY afordancji
> (nie aktor per jagoda) + spatial hash + EQS budżetowany + kręgosłup eksploracji. To realizacja
> kierunku „C++ food-registry" z TECH-11 (stan świata w „mózgu", nie w BP). Weryfikacja w E:\Game (5.7);
> plan portu do Game_58 (5.8 live) po domknięciu hotspotów.

| ID | Task | Status | Prio | Depends | Notes |
|---|---|---|---|---|---|
| L0-TA-S1 | Affordance CORE: `UWorldAffordanceSubsystem` (rejestr + spatial hash 2D + regen na timerze + atomowy claim + cancel-on-death) | ✅ | P1 | L3-01 | **CORE DONE + PIE-verified 2026-06-29** (raport `l0_ta_s1_core_pie_verify`, commity 60282fd/0d6c1bf/fb5db96/0a42c1c/e6d2999). Rekordy afordancji (Type/Yield/RegenPerHour/ColdDampen), `Register`/`Consume`/`QueryNearest`/`TryReserve`/`Release`, `GatherAffordanceLocations`. Twarde liczby: 3/3 rejestracja (id nie-recyklowane), dane 1:1 edytor→rejestr, spatial hash cell=(-1,-1), **regen na timerze** (RemainingYield 10→40→50 clamp), atomowy claim (podwójna rezerwacja blokowana, query pomija zajęte), cancel-on-death (weak-ptr lazy-validate = slot wolny NATYCHMIAST + RegenTick-backstop). |
| L0-TA-S1b | `AffordanceSourceActor` (data-driven, self-register BeginPlay/EndPlay) | ✅ | P1 | L0-TA-S1 | **DONE + PIE-verified 2026-06-29** (dodatek spoza §1 briefu, ratyfikowany 0d6c1bf). 3 aktory testowe register 1:1. |
| L0-TA-S1c | `UEnvQueryGenerator_Affordances` (custom EQS generator: typ+radius z subsystemu → itemy Point) | ✅ | P1 | L0-TA-S1 | **DONE + PIE-verified 2026-06-29 na UE 5.8** (`EQS_FindResource`, `EQS_OUTPUT_COUNT=3`, 3 zarejestrowane afordancje jako punkty). Zamyka 4. punkt DoD briefu. |
| L0-TA-S1d | `UEQSBudgetManager` (throttle MaxConcurrent=12, kolejka pending, cancel-on-death zwalnia slot) | ✅ | P1 | L0-TA-S1 | Kod DONE + GAME/Editor build zielony. `LogEQSBudget`. **Test współbieżności/throttle live = odłożony** (osobny test, §POZOSTAŁE raportu). |
| L0-TA-S1e | Kręgosłup eksploracji: BTTaski (`RollWanderDirection`/`MoveToKnownZone`/`AttachToSafeZone`/`Eat`/`Drink`/`RunBudgetedEQS`) + PATCH CaldrethZone (`bIsSafeZone`, `ShelterColdDampen`, `AssignedMembers`, `RegisterMember`) + `GetFeltTemperature()` | 🔨 | P1 | L0-TA-S1c | Kod BTTasków + patche DONE + build zielony. **Pełna pętla autonomiczna** (homeless→wander→EQS→attach→forage→spadek L1) = NIEzweryfikowana — wymaga autoringu poddrzewa BT + świeże PIE. Body-coupling felt-temp→CurrentTemp ODROCZONY (osobny thermo-slice). |
| L0-TA-S2 | Port afordancji/EQS/BT → E:\Game_58 (5.8 live target) | ⬜ | P1 | L0-TA-S1e | Po domknięciu hotspotów. E:\Game (5.7) = env weryfikacyjny (natywny MCP 5.8 umiera na cyklu PIE). |
...
| L1-06 | Sleep model: HoursAwake increment timer | ✅ | P1 | L1-03 | **DONE — Sleep ETAP 1 (commit a7efcdb) + TASK 2 (4e05686).** HoursAwake rośnie na timerze metabolizmu (bez Tick), EFatigueState, reset przy śnie. Sen dobrowolny + auto-wake z pułapki omdlenia PIE-verified 2026-06-23. |
| L1-07 | Sleep debuffs (fog -30%, microsleeps, passout ragdoll) | ✅ | P1 | L1-06 | **DONE — Sleep ETAP 2.** Mgła liniowa 1.0@16h→0.70@24h, mikrosen (FRand ≥20h), omdlenie (StopLogic + `OnCollapse`), buffy przez BlueprintImplementableEvent. PIE-verified. |
| L1-08 | Rested buff (+20% work/eureka) | ✅ | P2 | L1-06 | **DONE — Sleep ETAP 2** (`bIsRested`, `RestedWorkBonus=0.2`, WorkEff=1.2 po wyspaniu). ⚠️ DRIFT: commity oznaczone „L1-08" (1e2d0a8/adb1050) dotyczą OSOBNEJ funkcji **metabolizm→prędkość ruchu** (stamina scale + sleep/microsleep freeze), NIE tego wiersza — patrz L1-08b niżej. |
| L1-08b | Metabolic state → movement speed (stamina scale + sleep freeze) | ✅ | P2 | L1-06 | **DONE + PIE-verified 2026-06-23 (commit 1e2d0a8, raport `l108_stamina_speed`).** Prędkość skaluje się kondycją; sen/mikrosen zamrażają ruch; gradient wg składu ciała. (TECH-10 blokował live-magnitude — zamknięte jako misdiagnoza.) NOWY wiersz — wcześniej bezimienny feature. |
| L1-09 | Temperature system (day/night + fire/clothing) | ✅ | P1 | L1-01, L2-01 | **DONE + PIE-verified.** AmbientTemp (strefy) + warstwa doby (cold ×fat-burn, hipotermia stopniowa) + **L1-09a clothing** (izolacja additive-clamped, InsulationFactor 0.88 nagi→0.28 ubrany, commit 0ab692b) + **L1-09b fire** (heat-source registry + component, radiant→AmbientTemp, huddle falloff+clamp, commit 2395d6d). Raporty `l109a_clothing_done`, `l109b_fire_done`. |
| L1-10 | Thirst activity scaling (rest×1/work×3/combat×5) | ⬜ | P2 | L1-01 | **INERT — zweryfikowane 2026-06-30 (`REPORTS/raport_2026-06-30_l110_thirst_dead.md`).** Mechanizm spalania podpięty+poprawny (`burn 2.0/tick × CurrentActionHydrationMultiplier`, cpp:172), ale **sterownik MARTWY**: `SetActionByRow`/`SetCurrentActionMultiplier` bez żadnego wołającego (C++ i BP), `ActionCostTable=nullptr`, `DT_ActionCosts` odpięty → mnożnik na stałe **1.0** w grze. PIE-dowód: oba NPC spalają wodę identycznie mimo ruchu; iniekcja `mult=5` → C_1 drenuje ~5× szybciej (C_2 −12.0 vs C_1 ≥−58.0 w 62.66s). = ten sam odpięty drut co PROJECT_STATE §5 Problem 1. **Naprawa = GATE ARCHITEKTA** (wołać SetActionByRow z BT/metabolizmu + przypisać tabelę + wiersze rest/work/combat). |
...
| L3-05 | P2P contract system (FContract, virtual pool) | 🔨 | P1 | L3-01, L2-01 | **RDZEŃ DONE + PIE-verified 2026-06-24 (commity 65e1be6, a28bd54; raport `l305_p2p_slice1`).** `Social/ContractTypes.h` (`EContractStatus`, `FContract`), `UContractPoolSubsystem` (WorldSubsystem: `PostContract`/`GetOpenContracts`/`AcceptAndFulfill`=wirtualny swap z roll-backiem, resolve po NPCId, mutatory BlueprintCallable), `UContractTraderComponent` (driver per-NPC, timer), helpery barteru w InventoryComponent, `Inventory.OwnerID=NPCId`. Swap PIE-verified twardymi liczbami (FurCoat↔Berries, Open→Fulfilled). **BLOKER (autonomiczna pętla):** `UContractTraderComponent` NIE jest jeszcze na `BP_NPC_Character` → brak NPC handlującego SAM z potrzeby. Wymaga decyzji dyrektora (dodanie komponentu do defaultów BP → ręczny Ctrl+S). Odłożone slice 2+: fizyczny MoveTo, ceny/negocjacje, reputacja, zonal pool. |
| L3-06 | Contract BT tasks (offer/accept/fulfill barter) | ⬜ | P1 | L3-05 | Driver `UContractTraderComponent` napisany (rdzeń L3-05), ale NIE wpięty na pawna + brak gałęzi BT. = ten sam bloker co L3-05 (komponent na BP + świeże PIE autonomicznego obrotu). |
