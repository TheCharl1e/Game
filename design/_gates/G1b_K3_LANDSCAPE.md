...
## 5. NAV INVOKERS (dyrektor: ruszaj sekwencję) — IMPLEMENTACJA, rebuild pending
- **Config** `Config/DefaultEngine.ini` `[NavigationSystemV1]`: **`bGenerateNavigationOnlyAroundNavigationInvokers=True`** — nav buduje się tylko wokół invokerów (nie cała 10-km wyspa).
- **`NPCSpawner` (C++, patch):**
  - Spawner = invoker (`UNavigationInvokerComponent` w konstruktorze) — **seeduje nav wokół strefy spawnu**, łamie deadlock invoker-only (bez tego `GetRandomReachablePointInRadius` nie ma nav do próbkowania na starcie).
  - Krótki `SpawnDelay` (timer) przed falą — nav zdąży się zbudować wokół spawnera.
  - **Każdy NPC dostaje `UNavigationInvokerComponent`** (gen 3000 / removal 5000 uu) → dynamiczny nav podąża za NPC po 10 km; drobne cele afordowalne bo obszar ograniczony.
  - Moduł `NavigationSystem` już w Build.cs Stan_Pierwotny → bez zmian Build.cs.
...
- **DŁUG WIZUALNY — CZĘŚCIOWO NAPRAWIONY:** mapa miała **ZERO świateł** (0 DirectionalLight/SkyLight/SkyAtmosphere) + Landscape bez materiału → czerń. Dodano **DirectionalLight (movable) + SkyLight (real-time capture) + SkyAtmosphere + materiał Landscape (BasicShapeMaterial)** → **scena renderuje** (niebo, chmury, oświetlony teren — potwierdzone zrzutem). Mapa zapisana.
- **BLOKER WIZUALNY (tooling MCP):** `capture_viewport` zwraca ZBUFOROWANY kadr gdy PIE aktywne (ignoruje `set_level_viewport_camera_info`); w plain-editor kadruje. `HighResShot` nie produkuje pliku.
- **BLOKER WIZUALNY #2 (ważny — dotyczy Opcji C):** **Landscape z C++ `ALandscape::Import` NIE renderuje się wizualnie** w edytorze (kolizja/nav/NPC działają — traces trafiają, 50/50 spawn — ale powierzchnia niewidoczna: pusta siatka + ikonki). Nie Nanite (enable_nanite=False). WorldGridMaterial ani BasicShapeMaterial nie pomaga. → **Import C++ buduje komponenty+kolizję, ale render-state jest niepełny.** To realny dług Opcji C. Podejrzenie: brakuje kroku budowy render-danych heightmapy / edytorowego „build". **Manualny import (Opcja A) renderuje niezawodnie** — kandydat na fix wizualny.
- **Wniosek dla wideo:** funkcjonalnie działa, ale bez widocznego terenu nie ma sensownego nagrania. Do rozstrzygnięcia: (a) odłożyć wizual, (b) re-import Landscape manualnie (Opcja A → widoczny), (c) debug render-state C++ importu.
