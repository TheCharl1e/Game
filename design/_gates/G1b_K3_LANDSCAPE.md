...
- **BLOKER WIZUALNY (tooling MCP):** `capture_viewport` zwraca ZBUFOROWANY kadr (ignoruje `set_level_viewport_camera_info`), `HighResShot` nie produkuje pliku → **nie da się wykadrować/zeskryptować sekwencji klatek przez MCP**. Wideo z widocznymi NPC = nagranie OBS/Win+G (scena renderuje w żywym edytorze) z moją synchronizacją timingu PIE.
